...
// ─── USER MANAGER (Progreso Avanzado FASE 1) ─────────────────────────────────
const UserManager = {
  data: {
    totalTests: 0,
    totalCorrect: 0,
    ...
    lastState: null,       // { permit, testNum, topic, currentQuestion, answers }
    lastPermit: null       // Last used permit
  },
  load() {
    try {
  ...
  checkDaily() {
    ...
    if (this.data.lastActiveDate !== today) {
      if (this.data.lastActiveDate) {
        ...
        if (diffDays > 1) this.data.streak = 0; // Perdió racha
        else if (diffDays === 1 && this.data.dailyQuestions >= 30) {
           // Continua racha
        }
      }
      this.data.dailyQuestions = 0; // Reset diario
    }
  },
  recordActivity() {
    const today = new Date().toISOString().split('T')[0];
  ...
  recordQuestionResult(permit, topic, qId, isCorrect) {
    ...
    // Topic logic
    if(!this.data.topicStats[permit]) this.data.topicStats[permit] = {};
    const t = topic || 'general';
    if(!this.data.topicStats[permit][t]) this.data.topicStats[permit][t] = { correct: 0, total: 0 };
    this.data.topicStats[permit][t].total++;
    if(isCorrect) this.data.topicStats[permit][t].correct++;

    this.recordActivity();
  },
  saveLastState(permit, testNum, topic = null, currentQ = 0, ans = {}) {
    this.data.lastState = { permit, testNum, topic, currentQuestion: currentQ, answers: ans };
    this.data.lastPermit = permit;
    this.save();
  },
  updateUI() {
...
async function showFavoritesScreen() {
  ...
  if(favQs.length === 0) { list.innerHTML = '<div style="text-align:center">No encontradas en BD.</div>'; return; }

...
const UserManager = {
  data: {
    ...
    mistakes: [],          // Array of Q IDs that user got wrong
      unlockedSigns: [],     // Freemium unlocked signals
...
window.showAppAlert = showAppAlert;
  ...
  function showAppUnlockModal(id, title) {
      let overlay = document.getElementById('unlockAlertOverlay');
      if (!overlay) {
          overlay = document.createElement('div');
          overlay.id = 'unlockAlertOverlay';
          overlay.className = 'modal-overlay';
          overlay.style.zIndex = '100000';
          overlay.innerHTML = `
              <div class="modal-content" style="max-width: 350px; text-align: center; padding: 30px 20px;">
                  <h3 style="margin-top:0; font-size: 20px; color: var(--text);">Señal Bloqueada</h3>
                  <p style="color: var(--text2); margin: 15px 0 25px;">Esta señal es exclusiva. Desbloquéala para siempre.</p>
                  
                  <button class="primary-btn" onclick="window.triggerPaywall('premium')" style="width: 100%; margin-bottom: 12px; background: linear-gradient(135deg, #ffc107, #ff9800); color: #000;">👑 Hacerme Premium</button>
                  
                  <button class="primary-btn" id="btnWatchVideoAd" style="width: 100%; margin-bottom: 15px; background: #333;">🎬 Ver Vídeo (Desbloquear)</button>
                  
                  <div style="cursor:pointer; color: var(--text2); font-size:14px;" onclick="document.getElementById('unlockAlertOverlay').classList.remove('active')">Cancelar</div>
              </div>
          `;
          document.body.appendChild(overlay);
      }
      
      const btnVideo = document.getElementById('btnWatchVideoAd');
      btnVideo.onclick = () => {
  ...
  window.showAppUnlockModal = showAppUnlockModal;
  
  function simulateRewardedAd(id) {
      // Hide unlock modal
      document.getElementById('unlockAlertOverlay').classList.remove('active');
      
      // Show loading ad spinner
      let adOverlay = document.getElementById('adAlertOverlay');
      if (!adOverlay) {
          adOverlay = document.createElement('div');
          adOverlay.id = 'adAlertOverlay';
          adOverlay.className = 'modal-overlay';
          adOverlay.style.zIndex = '100005';
          adOverlay.innerHTML = `
              <div class="modal-content" style="max-width: 300px; text-align: center; padding: 40px 20px; background: #000; border: 1px solid #333;">
                  <div style="font-size: 40px; margin-bottom: 20px; animation: spin 2s linear infinite;">⏳</div>
                  <h3 style="margin-top:0; font-size: 18px; color: #fff;">Anuncio Patrocinado...</h3>
                  <p style="color: #aaa; margin: 10px 0 0; font-size: 14px;">Por favor, espera 5 segundos.</p>
      ...
      setTimeout(() => {
          ...
          // Reward the user
          if (!UserManager.data.unlockedSigns) UserManager.data.unlockedSigns = [];
          if (!UserManager.data.unlockedSigns.includes(id)) {
              UserManager.data.unlockedSigns.push(id);
              UserManager.save();
          }
          
          showAppAlert('¡Señal Desbloqueada!', 'Has desbloqueado esta señal para siempre.');
          
          // Re-render current tab to remove the lock
          const activeTab = document.querySelector('.sign-tab.active');
          if (activeTab) {
              const sg = document.getElementById('signsGrid');
              if(sg) sg.innerHTML = '';
              // we don't have direct access to renderSigns from outside if it's not window?
              // but simulateRewardedAd is in the same scope!
              // actually wait, simulateRewardedAd is global scope here?
              // Let's just dispatch click on active tab
              activeTab.click();
          }
      }, 5000);
  }
...
async function renderMemoSelectors() {
    ...
    let html = `<div style="display:flex; gap:10px; margin-top:10px;">
        <select id="memoPermitSelect" onchange="handleMemoPermitChange(this.value)" style="flex:1; padding:12px 14px; border-radius:10px; border:1px solid var(--border); background-color: var(--card-bg); color: var(--text); font-size:16px; font-weight: 500; appearance: none; -webkit-appearance: none; background-image: url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23aaaaaa%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E'); background-repeat: no-repeat; background-position: right 14px center; background-size: 12px auto; outline: none; cursor: pointer;">`;
    ...
    html += `<select id="memoTopicSelect" onchange="handleMemoTopicChange(this.value)" style="flex:1; padding:12px 14px; border-radius:10px; border:1px solid var(--border); background-color: var(--card-bg); color: var(--text); font-size:16px; font-weight: 500; appearance: none; -webkit-appearance: none; background-image: url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23aaaaaa%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E'); background-repeat: no-repeat; background-position: right 14px center; background-size: 12px auto; outline: none; cursor: pointer;">`;
